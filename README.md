...
| [Compare-GPOsByOU](Compare-GPOsByOU/) | 1.3.0 | PowerShell | Reports all GPOs linked to each OU including link order, enabled/enforced state, WMI filter details, and orphaned GPOs. Supports DC pinning, progress reporting, settings conflict detection (with PS 7+ parallel fetching), and `-PassThru` for pipeline use. |
