...
| [Compare-GPOsByOU](Compare-GPOsByOU/) | 1.4.0 | PowerShell | Reports all GPOs linked to each OU including link order, enabled/enforced state, WMI filter details, and orphaned GPOs. Supports DC pinning, parallel inheritance fetching on PS 7+, settings conflict detection, and `-PassThru` for pipeline use. |
