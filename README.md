...
| [CleanupPolicies](CleanupPolicies/) | 1.4.0 | PowerShell | Inventories Group Policy Objects that are unlinked (not applied to any OU, site or domain container) or have no 'Apply Group Policy' Allow ACE, producing an Excel/CSV report for GPO cleanup reviews. |
| [Compare-GPOsByOU](Compare-GPOsByOU/) | 1.6.0 | PowerShell | Reports all GPOs linked to each OU including link order, enabled/enforced state, WMI filter details, and orphaned GPOs. Supports DC pinning, parallel inheritance fetching on PS 7+, settings conflict detection, and `-PassThru` for pipeline use. |
...
| [Get-AllGPOSettings](Get-AllGPOSettings/) | 1.1.0 | PowerShell | Reports every configured setting in every GPO in a domain, regardless of where (or whether) each GPO is linked, including orphaned GPOs. Exports a single Excel/CSV "Settings" report with parallel report fetching on PS 7+ and `-PassThru` for pipeline use. |
...
| [Shared](Shared/) | 1.1.0 | PowerShell module | Shared logging (`Initialize-ScriptLog`, `Write-ScriptLog`, `Remove-OldLog`), module-import (`Import-RequiredModule`), and GPO XML report parsing (`Get-XmlInnerText`, `Get-GpoSetting`) utilities used by PowerShell scripts in this repository. |
